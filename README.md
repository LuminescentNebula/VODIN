...
## TODO:
- **Клиент**
  - Зависит от сети, даже если сети нет должен все равно рабоать
  - Не перезапускается при ошибках
  - Долго отдает info, что требует больше времени на сканирование
  - Не отправляется версия veyon
  - Убрать запоминания мастера. Просто веб сервер, который отдает нуэную информацию
  - Не работает на linux
  - Убрать подтверждение через ключ
  - Убрать лишнее из config
- **Мастер**
  - Название сети не должно быть связано с диапазоном, по которому идет сканирование
  - Не работает с истечением ip клиентов
  - Заменяет все таблицу hosts, а не только измененные и конфликты
  - Не работает на windows
  - Сканирование при обращении на endpoint - это шиза. Как он это вообще придумал?
  - Veyon не должен перезапускаться, вместо этого в hosts или в DNS будут вписываться ip, которые соответсвуют адресу клиенту и имени клиента.local
  - Сделать список найденных клиентов асинхронным, потому что поиск клиентов асинхронный
  - Убрать лишнее из config
